...
export default function App() {
  ...
  useEffect(() => {
    homeDir()
      .then(async (p) => {
        ...
        setHome(normalized);
        try {
          await invoke("workspace_authorize", { dir: normalized });
        } catch {
  ...
  const switchWorkspace = useCallback(
    async (env: WorkspaceEnv) => {
      ...
      if (nextHome) {
        try {
          await invoke("workspace_authorize", { dir: nextHome });
        } catch {
  ...
  useEffect(() => {
    invoke<string | null>("workspace_current_dir")
      .then(setLaunchCwd)
  ...
  const openGitGraphFromContext = useCallback(async () => {
    ...
    try {
      const repo = await invoke<{ repoRoot: string; branch: string | null } | null>("git_resolve_repo", { path: sourceControlContextPath });
      if (!repo) return;
